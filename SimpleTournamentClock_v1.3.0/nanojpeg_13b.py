...
#
# Further mods by Mayur Patel June 26 2012
...
# nj_context_t now known as simply NJ
class NJ(object):
    def init(self):
        self.error = 0
...
def njSkipBits(nj, bits):
    if (nj.bufbits < bits):
        njShowBits(bits)
    nj.bufbits -= bits

def njGetBits(nj, bits):
    res = njShowBits(nj, bits)
    njSkipBits(nj, bits)
    return res
...
def njDecode16(nj, pos):
    return (nj.spos[pos] << 8) | nj.spos[pos + 1]

def njDecodeLength(nj):
    if (nj.size < 2):
        raise Exception(NJ_SYNTAX_ERROR)
    nj.length = njDecode16(nj, nj.pos)
...
def njDecodeSOF(nj):
    ...
    nj.height = njDecode16(nj, nj.pos + 1)
    nj.width = njDecode16(nj, nj.pos + 3)
...
def njDecodeDRI(nj):
    ...
    if (nj.length < 2):
        raise Exception(NJ_SYNTAX_ERROR)
    nj.rstinterval = njDecode16(nj, nj.pos)
    njSkip(nj, nj.length)

#code is an array with one element, since we need to return the code to the caller
def njGetVLC(nj, vlc, code):
    value = njShowBits(nj, 16)
    bits = vlc[value].bits
    if not bits:
        raise Exception(NJ_SYNTAX_ERROR)
    njSkipBits(nj, bits)
...
def njDecodeBlock(nj, c, sout, out):
    njZZ = ( 0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40, 48, 41, 34, 27, 20, 13, 6, 7, 14, 21, 28, 35, 42, 49, 56, 57, 50, 43, 36, 29, 22, 15, 23, 30, 37, 44, 51, 58, 59, 52, 45, 38, 31, 39, 46, 53, 60, 61, 54, 47, 55, 62, 63 )
    code = [0]
    value = 0
    coef = 0
    nj.block = [0 for i in nj.block]
    # @@ for i in range(len(nj.block)):
    # @@    nj.block[i] = 0
    c.dcpred += njGetVLC(nj,nj.vlctab[c.dctabsel], None)
    nj.block[0] = c.dcpred * nj.qtab[c.qtsel][0]
    while True: # do {
        value = njGetVLC(nj,nj.vlctab[c.actabsel], code);
...
def njUpsampleH(c):
    ...
    y = c.height
    while y:
        out[lout + 0] = ((139 * c.pixels[lin + 0] + -11 * c.pixels[lin + 1])+64)>>7
        out[lout + 1] = ((104 * c.pixels[lin + 0] + 27 * c.pixels[lin + 1] + -3 * c.pixels[lin + 2])+64)>>7
        out[lout + 2] = ((28 * c.pixels[lin + 0] + 109 * c.pixels[lin + 1] + -9 * c.pixels[lin + 2])+64)>>7
        for x in range(xmax):
            out[lout + (x << 1) + 3] = ((-9 * c.pixels[lin + x] + 111 * c.pixels[lin + x + 1] + 29 * c.pixels[lin + x + 2] + -3 * c.pixels[lin + x + 3])+64)>>7
            out[lout + (x << 1) + 4] = ((-3 * c.pixels[lin + x] + 29 * c.pixels[lin + x + 1] + 111 * c.pixels[lin + x + 2] + -9 * c.pixels[lin + x + 3])+64)>>7
        lin += c.stride
        lout += c.width << 1
        out[lout - 3] = ((28 * c.pixels[lin - 1] + 109 * c.pixels[lin - 2] + -9 * c.pixels[lin - 3])+64)>>7
        out[lout - 2] = ((104 * c.pixels[lin - 1] + 27 * c.pixels[lin - 2] + -3 * c.pixels[lin - 3])+64)>>7
        out[lout - 1] = ((139 * c.pixels[lin - 1] + -11 * c.pixels[lin - 2])+64)>>7
        y -= 1
    c.width <<= 1
    c.stride = c.width
    c.pixels = out
...
def njUpsampleV(c):
    ...
    for x in range(w):
        cin = x
        cout = x
        out[cout] = ((139 * c.pixels[cin] + -11 * c.pixels[cin + s1])+64)>>7
        cout += w
        out[cout] = ((104 * c.pixels[cin] + 27 * c.pixels[cin + s1] + -3 * c.pixels[cin + s2])+64)>>7
        cout += w
        out[cout] = ((28 * c.pixels[cin] + 109 * c.pixels[cin + s1] + -9 * c.pixels[cin + s2])+64)>>7
        cout += w
        cin += s1
        y = c.height - 3
        while y:
            out[cout] = ((-9 * c.pixels[cin - s1] + 111 * c.pixels[cin] + 29 * c.pixels[cin + s1] + -3 * c.pixels[cin + s2])+64)>>7
            cout += w
            out[cout] = ((-3 * c.pixels[cin - s1] + 29 * c.pixels[cin] + 111 * c.pixels[cin + s1] + -9 * c.pixels[cin + s2])+64)>>7
            cout += w
            cin += s1
            y -= 1
        cin += s1
...
def njDecode(nj, jpeg, size):
    ...
    while not nj.error:
        ...
        njSkip(nj,2)
        m = nj.spos[nj.pos - 1]
        if m == 0xC0: njDecodeSOF(nj)
        elif m == 0xC4: njDecodeDHT(nj)
        elif m == 0xDB: njDecodeDQT(nj)
        elif m == 0xDD: njDecodeDRI(nj)
        elif m == 0xDA: njDecodeScan(nj)
        elif m == 0xFE: njSkipMarker(nj)
        elif (m & 0xF0) == 0xE0:
            njSkipMarker(nj)
        else:
